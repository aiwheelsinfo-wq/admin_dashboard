...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Basic validation
    if (!$company_name || !$company_owner_name || !$contact_person || !$contact_number || !$email || !$gst_number || !$password) {
        ...
        try {
            ...
            if (mysqli_stmt_num_rows($check_stmt) > 0) {
                ...
                $stmt = mysqli_prepare($conn, 
                    "INSERT INTO partners (partner_name, company_name, company_owner_name, contact_person, mobile_number, email, gst_number, password, status, notes)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)"
                );
                
                mysqli_stmt_bind_param($stmt, 'sssssssss', 
                    $partner_name, $company_name, $company_owner_name, $contact_person, $contact_number, $email, $gst_number, $hashed_password, $notes
                );
                ...
                if (mysqli_stmt_execute($stmt)) {
                    ...
                    $_SESSION['partner_company'] = $company_name;
...
<body>
    ...
    <div class="console-container">
        ...
        <div class="header-section">
            <h2 class="header-title">Register Company Account</h2>
            <p class="header-subtitle">Join the Redox API Service. Complete your developer details and set an access password to create your account.</p>
        </div>
        ...
        <form method="POST">
            <div class="form-grid">
                ...
                <div class="form-group full-width">
                    <label class="form-label" for="password"><i class="fa-solid fa-lock"></i> Account Password <span>*</span></label>
                    <input type="password" id="password" name="password" class="form-control" placeholder="Min. 6 characters" required>
                </div>
            </div>

            <button type="submit" class="btn-submit">
                <i class="fa-solid fa-user-plus"></i> Submit & Register
            </button>
        </form>
