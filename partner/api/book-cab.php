...
if (!function_exists('validate_local_taxi_boundary')) {
...
// Enforce Local Taxi City Boundary limits
if ($trip_type === 'Local-taxi' || $trip_type === 'Local-Taxi') {
    if (!validate_local_taxi_boundary($from, $to)) {
        $msg = "Local Taxi rides must stay within the same city boundaries (e.g. Pune City limits). Please book One-way for intercity trips.";
        log_api_request($partner['id'], $_API_NAME, $body, ['status'=>false,'message'=>$msg], 'error');
        api_error($msg, 400);
    }
}
