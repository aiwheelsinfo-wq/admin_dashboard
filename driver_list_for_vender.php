...
switch ($method) {
  ...
  SELECT 
    d.driver_id,
    d.phone_number,
    COALESCE(NULLIF(dl.holder_name, ''), d.full_name) AS full_name,
    d.status,
    d.latitude,
    d.longitude,
    d.insurnce_doe,
...
JOIN drivers d 
    ON dv.driver_id = d.phone_number
LEFT JOIN driver_dl_verifications dl 
    ON UPPER(REPLACE(REPLACE(d.license_no, ' ', ''), '-', '')) = dl.dl_number
LEFT JOIN bookings b 
    ON b.driver_id = d.phone_number
    AND b.date >= CURDATE()
WHERE dv.vendor_id = ?
